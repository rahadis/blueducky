...
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QLabel, QVBoxLayout, QWidget, QFileDialog, QMessageBox, QComboBox,  QListWidget
from bluetooth import discover_devices
from utils.menu_gui import ( read_duckyscript, run, restart_bluetooth_daemon)
...
def run_gui_attack(target_address, payload_path, adapter_id='hci0'):
    setup_logging()
    log = logging.getLogger(__name__)

    if not target_address or not os.path.exists(payload_path):
        return "Target address or payload not valid"

    duckyscript = read_duckyscript(payload_path)
    if not duckyscript:
    ...
    while True:
        try:
            ...
            try:
                subprocess.run(['bluetoothctl'], input=f"remove {target_address}\n", text=True)
            except Exception as cleanup_error:
                log.warning(f"Cleanup error: {cleanup_error}")
            terminate_child_processes()




class BluetoothHIDApp(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.initUI()
        self.target_address = None
        self.check_bluetooth_status()  # Periksa status Bluetooth saat aplikasi dimulai

    def initUI(self):
        self.setWindowTitle('Bluetooth HID Attack Tool')
        self.setGeometry(100, 100, 500, 400)

        layout = QVBoxLayout()

        # Status Bluetooth
        self.bluetooth_status_label = QLabel('Status Bluetooth: Memeriksa...')
        layout.addWidget(self.bluetooth_status_label)


        self.step_label = QLabel('Scan for Bluetooth Devices')
        layout.addWidget(self.step_label)

        self.device_label = QLabel('Available Bluetooth Devices:')
        ...
        # Status perangkat yang dipilih
        self.device_status_label = QLabel('Status: Tidak ada perangkat yang dipilih.')
    ...
    def check_bluetooth_status(self):
        try:
            # Periksa status Bluetooth dengan perintah sistem
            result = subprocess.run(['hciconfig'], capture_output=True, text=True)
            if "UP RUNNING" in result.stdout:
                self.bluetooth_status_label.setText('Status Bluetooth: ✅ ON')
                self.bluetooth_status_label.setStyleSheet('color: green')
            else:
                self.bluetooth_status_label.setText('Status Bluetooth: ❌ OFF')
                self.bluetooth_status_label.setStyleSheet('color: red')
        except FileNotFoundError:
            self.bluetooth_status_label.setText('Status Bluetooth: ❌ NOT FOUND')
            self.bluetooth_status_label.setStyleSheet('color: red')
            
    def select_device(self):
        selected_device = self.device_list.currentItem()
        if selected_device:
            self.target_address = selected_device.text().split(" ")[0]  # Extract the address from the selected item
            self.device_status_label.setText(f'Status: Device terpilih - {self.target_address}')  # Perbarui status perangkat
            QMessageBox.information(self, 'Device Selected', f'Selected Device: {self.target_address}')
            self.start_button.setEnabled(True)  # Enable the start button after selecting a device
        else:
            QMessageBox.warning(self, 'Warning', 'No Bluetooth device selected.')
    ...
    def update_device_list(self, devices):
        ...
        if self.device_list.count() > 0:
            ...
            QMessageBox.warning(self, 'Warning', 'No Bluetooth devices found. Please ensure Bluetooth is enabled and discoverable.')
    ...
    def load_payloads(self):
        ...
        self.payload_combo.addItems(payloads)
            
    def start_attack(self):
        selected_payload = self.payload_combo.currentText()
        if not selected_payload:
            QMessageBox.warning(self, 'Warning', 'No payload selected.')
            return
        ...
        script_directory = os.path.dirname(os.path.realpath(__file__))
        payload_path = os.path.join(script_directory, 'payloads', selected_payload)
        
        command = f"python3 CLI.py --adapter hci0 --target {self.target_address} --payload \"{payload_path}\""
        terminal_cmd = [
            "gnome-terminal",
            "--",
            "bash",
        ...
        # command = ["xfce4-terminal", "--hold", "--command", f"python3 attack.py --adapter hci0 --target {self.target_address} --payload \"{payload_path}\""]
        # subprocess.Popen(command)

        # Pastikan path payload di-escape kalau mengandung spasi
        #command = f"python3 CLI.py --adapter hci0 --target {self.target_address} --payload \"{payload_path}\""


        #os.system(f"xfce4-terminal --hold --command='{command}'")
...
class BluetoothScanner(QThread):
    ...
    def run(self):
        ...
        for addr, name in devices:
            formatted_devices.append(f"{addr} {name}")

        self.devices_found.emit(formatted_devices)
