...
from utils.menu_functions import ( read_duckyscript, run, restart_bluetooth_daemon, get_target_address)
...
# digunakan untuk menambahkan level log baru antara level standar yang ada di Python, antara level INFO dan WARNING
NOTICE_LEVEL = 25

# custom formatter class yang menambahkan warna khusus untuk log level NOTICE
class ColorLogFormatter(logging.Formatter):
    COLOR_MAP = {
        ...
        NOTICE_LEVEL: AnsiColorCode.BLUE,  # Color untuk NOTICE level
    }
...
# Menambahkan method custom seperti notice ke dalam kelas Logger 
def notice(self, message, *args, **kwargs):
    if self.isEnabledFor(NOTICE_LEVEL):
        self._log(NOTICE_LEVEL, message, args, **kwargs)

# Menambahkan custom level log dan method pada modul logging 
logging.addLevelName(NOTICE_LEVEL, "NOTICE")
logging.Logger.notice = notice

# Mengatur logging dengan color formatter dan custom log level
def setup_logging():
    ...
    # Untuk mengatur level logging ke INFO sehingga hanya pesan dengan level INFO yang dicatat
...
class Adapter:
    ...
    def set_property(self, prop, value):
        # Mengonversi sebuah nilai menjadi string jika nilai tersebut bukan sudah dalam bentuk string
        value_str = str(value) if not isinstance(value, str) else value
        #command = ["hciconfig", self.iface, prop, value_str]
        command = ["sudo", "hciconfig", self.iface, prop, value_str]
        self._run_command(command)

        # Memverifikasi apakah sebuah properti telah diset dengan benar
    ...
    def enable_ssp(self):
        try:
            # mengaktifkan SSP (Secure Simple Pairing) pada perangkat Bluetooth
            #  perintah placeholder yang perlu diganti dengan perintah yang sebenarnya.
            #ssp_command = ["hciconfig", self.iface, "sspmode"]
...
class L2CAPClient:
    ...
    def reconnect(self):
        # Memberi Notifikasi pada proses reconnection
    ...
    def send(self, data):
        ...
        # Untuk menambahkan timestamp ke dalam pesan log 
        ...
        try:
            ...
            self.reconnect()
            self.send(data)  # melakukan percobaan pengiriman kembali
    ...
    def connect(self, timeout=None):
        ...
        try:
            ...
            # mendefinisikan warna di logging
            red = "\033[91m"
            blue = "\033[94m"
            reset = "\033[0m"
    ...
    def send_keypress(self, *args, delay=0.0001):
        if args:
            ...
            # Jika tidak ada argumen, kirimkan laporan kosong
            self.send(self.encode_keyboard_input())
        time.sleep(delay)
        return True  # Tunjukkan bahwa pengiriman berhasil
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Run HID payload to target Bluetooth device.',
        epilog='Contoh: python3 CLI.py --adapter hci0 --target XX:XX:XX:XX:XX:XX --payload payloads/test.duc'
    )
    parser.add_argument('--adapter', default='hci0', help='Bluetooth adapter to use')
    parser.add_argument('--target', required=True, help='MAC address of the target device')
    parser.add_argument('--payload', required=True, help='Path to the duckyscript payload')
...
def main():
    ...
    print("📦 Argumen diterima:")
    print(f"   ➤ Adapter : {adapter_id}")
    print(f"   ➤ Target  : {args.target}")
    print(f"   ➤ Payload : {args.payload}")

    if not args.target or not args.payload:
        print("❌ ERROR: --target dan --payload harus disediakan saat dijalankan dari GUI.")
        sys.exit(1)

    target_address = args.target
    selected_payload = args.payload

    print(f"[INFO] Menjalankan langsung:")
    print(f"       ➤ Target   : {target_address}")
    print(f"       ➤ Payload  : {selected_payload}")
    ...
    while True:
        try:
            ...
            try:
                subprocess.run(['bluetoothctl'], input=f"remove {target_address}", text=True)
            except Exception as cleanup_error:
                log.warning(f"Cleanup error during unpairing: {cleanup_error}")
